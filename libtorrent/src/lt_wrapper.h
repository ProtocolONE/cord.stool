...
void* session_create2(int port_start, int port_end, int alert, int flags);
void session_close(void* ses);

// use TOR_* tags in tag list
int session_add_torrent(void* ses, int first_tag, ...);
int session_add_torrent_file(void* ses, const char* fn, const char* save_path);
int session_add_torrent_data(void* ses, const char* data, int size, const char* save_path);
...
int session_pop_alerts(void* ses, struct session_alert_t* alerts, int* count);
...
int torrent_get_status2(int tor, struct torrent_status* s);

// use SET_* tags in tag list
int torrent_set_settings(int tor, int first_tag, ...);
int torrent_set_setting(int tor, int tag, void* value);
